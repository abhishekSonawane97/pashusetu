...
### 8.4 Rate limiting implementation — Postgres token bucket (chosen)
...
1. **60/min/user write window (BR-090 #2):** table `rate_limits (key text primary key, count int, window_ends_at timestamptz)` with key `w:{userId}:{epochMinute}`. One statement per request: `INSERT ... ON CONFLICT (key) DO UPDATE SET count = rate_limits.count + 1 RETURNING count`; `count > 60` → 429 `RATE_LIMITED` with `details.retryAfterSeconds` = seconds to window end (FR-05). Expired rows removed by the daily GC job.
