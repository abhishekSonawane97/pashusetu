...
| P-2 | **Table names are plural snake_case** (`users`, `listings`, `listing_images`, `favorites`, `interest_events`, `reports`, `notifications`, `breeds`, `districts`) with **one canonical exception**: `moderation_log` (singular, as fixed by the foundation data model — "the log" is one ledger). | Never rename; other docs already reference these names. |
...
| sort_order | integer | no | — | CHECK 0–4; 0 = cover photo; max 5 rows per listing (service txn) | BR-023 |
...
Per BR-022, **every species carries a "Local / Crossbred" option** (गावठी / संकरित — the everyday farmer word गावठी is used first per the doc-10 register rule) so `breed_id` is always satisfiable. The canonical cattle/buffalo/goat/sheep lists are seeded verbatim; BULL_OX has no canonical list, so it is seeded with Maharashtra's draught cattle breeds (decision, recorded): Khillar (the state's signature bullock breed), Gir, Dangi, Gaolao, Deoni + Local / Crossbred.
...
|---|---|---|---|---|
| **Max 5 images per listing** | `CHECK (sort_order BETWEEN 0 AND 4)` (backstop: only 5 valid slots) | Attach endpoint counts existing images and inserts **in one transaction**; 6th attach → `PHOTO_LIMIT_EXCEEDED` | Transaction with count-then-insert; the CHECK bounds `sort_order` so even a bug cannot create slot 5+ | BR-023 |
| **Max 10 active listings per user** | none possible (cross-row count) | `POST /listings` runs `SELECT count(*) … WHERE seller_id=? AND status IN (DRAFT,PENDING,APPROVED,REJECTED,EXPIRED)` + insert **in one transaction** holding `SELECT … FOR UPDATE` on the seller's `users` row as a per-user mutex → `LISTING_LIMIT_REACHED` | The user-row lock serializes concurrent creates by the same seller; `listings_seller_idx` keeps the count instant | BR-024 |
...
| **Field ranges** (age 1–300, weight 5–1500, milk 0–60, lactation 0–15, description ≤ 1000, sort_order 0–4, view_count ≥ 0) | `CHECK` constraints (§9.2) | Same Zod schemas, which also enforce the min-length and species/sex conditionality (BR-022 matrix) that CHECKs cannot express | none needed | BR-022, BR-025 |
...
### 9.2 Hand-written SQL (added via `prisma migrate dev --create-only`, part of the initial migration set)
...
(No unique constraint on `listing_images(listing_id, sort_order)` — a deliberate decision: photo reordering swaps `sort_order` values in place, and a non-deferrable unique index would reject the intermediate state. The service layer rewrites the 0–4 sequence in one transaction; the CHECK bounds the slots.)
...
- [x] One fenced Prisma schema block containing all 10 models (User, District, Breed, Listing, ListingImage, Favorite, InterestEvent, Report, Notification, ModerationLog) and all 11 enums (Species, Sex, ListingStatus, LanguagePref, UserStatus, InterestType, ReportReason, ReportStatus, NotificationChannel, NotificationStatus, ModerationAction) with the exact canonical values
