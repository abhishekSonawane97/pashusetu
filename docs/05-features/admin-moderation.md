...
| Aspect | Value |
|---|---|
| Who | Users with `is_admin = true` only — provisioned manually in the DB by the Founder; **no UI or API grants/revokes the flag** (BR-012). The flag is set only via `pnpm grant-admin +91XXXXXXXXXX [--revoke]` (`scripts/grant-admin.ts`, requires `DIRECT_URL`) or raw SQL; every grant/revoke is noted in the ops journal (doc 13 §4.4). Canonical procedure: doc 07 §6.3 |
...
| Surface | Desktop-first layout at `/admin/*` in the same Next.js codebase (D1); reached from the home hamburger — `AppMenu` (`components/layout/AppMenu.tsx`) renders a **जाहिरात तपासणी** row linking to `/admin`, shown **only when `profile.isAdmin === true`**; still never in the bottom nav and never rendered for non-admins. The shipped admin chrome/tabs are Marathi-labelled (**पशुसेतू — प्रशासन** / **अ‍ॅप उघडा**), which diverges from this row's earlier **English-first UI** claim — the language decision is owned by doc 06 §5.3 (note the divergence there) |

## UX workflow

1. **S-18 gate:** admin opens `/admin`, logs in via phone OTP if needed; on success with `is_admin` verified → redirect to `/admin` — the review queue (S-19), formerly `/admin/pending`. The shipped chrome (`app/admin/layout.tsx`) is a **sticky top header** with three Marathi tabs: **रांग** (queue → `/admin`) · **आकडेवारी** (stats → `/admin/stats`, S-23) · **अभिप्राय** (feedback inbox → `/admin/feedback`). No Reports or Users tab ships and no 60-second badge polling; the अभिप्राय inbox surfaces an unhandled **`(N नवीन)`** count in its heading in place of the removed Pending/Reports badges.
...
8. **S-23 stats & audit:** `GET /admin/stats` (`lib/services/stats-service.ts`) renders read-only aggregates — **listings** (`total`, `byStatus`, `newToday`, `newWeek`); **views** (`total` = Σ `viewCount` over APPROVED listings, plus the **top 5 most-viewed** approved listings, each with species + breed (MR) + district (MR)); **interest** (Call / WhatsApp / Interest enquiry-event counts, all-time total and last-7-day total); **zeroEnquiryApproved** (count of APPROVED listings with no enquiry); and **topDistricts** by approved-listing count — no schema change and no server cache. The response contract is owned by doc 08 (API-34). Below it, `GET /admin/audit-log` renders the append-only `moderation_log` filterable by admin, action (`APPROVE|REJECT|BAN|UNBAN|RESOLVE_REPORT|DISMISS_REPORT|AUTO_HIDE`) and date range; system actions appear under the seeded "System" admin (BR-046).
...
| `GET /api/v1/admin/feedback?status=` | अभिप्राय inbox load — `requireAdmin`; default all, filter `NEW\|SEEN\|DONE` (owned by feedback.md + doc 08) |
| `PATCH /api/v1/admin/feedback/{id}` | Advance a feedback item NEW→SEEN→DONE ("पूर्ण झाले") — `requireAdmin` (owned by feedback.md + doc 08) |

## States

| State | What the admin sees |
|---|---|
| Loading | Table-row skeletons on S-19/S-21/S-22/S-23; inline spinner in the tapped action button; S-20 photo placeholders while object-storage image variants load. |
...
10. S-23 (`/admin/stats`) renders listings (total, by-status, new today/week), the approved view total + top-5 most-viewed, interest events by type (Call/WhatsApp/Interest) all-time and last-7-day, the zero-enquiry approved count, and top districts — read-only aggregates, admin-only, no schema change.
...
- Granting/revoking `is_admin` via UI or API — manual DB provisioning only: `pnpm grant-admin +91XXXXXXXXXX [--revoke]` (`scripts/grant-admin.ts`) or raw SQL, logged in the ops journal (BR-012, doc 07 §6.3, doc 13 §4.4).
...
- [x] Canonical `/admin/*` endpoints listed (API-25–API-34), plus the two shipped feedback endpoints (`GET /admin/feedback`, `PATCH /admin/feedback/{id}` — owned by feedback.md + doc 08); several of API-25–API-34 (reports/users/ban/audit-log) are not yet shipped; screens cited S-18–S-23 (+ S-08)
