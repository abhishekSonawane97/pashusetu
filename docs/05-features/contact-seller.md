...
4. On success, by type:
   ...
   - **INTEREST:** toast "विक्रेत्याला कळवले आहे" (the seller has been informed) + the seller's number shown on screen for manual dialing; the seller gets `NTF-INTEREST-RECEIVED` (in-app always; SMS per the 3/day/seller cap — BR-071, [notifications.md](notifications.md)).
5. Failure paths render per States (rate limit, unavailable listing, banned).
...
flowchart TD
    ...
    C1 -->|"Yes"| REQ["POST interest with type"]
    REQ --> C2{"Server result"}
    C2 -->|"200"| REV["interest_events row logged - phone and whatsappUrl returned"]
    REV -->|"CALL"| D1["Open tel link plus number sheet"]
    REV -->|"WHATSAPP"| D2["Open wa.me link with Marathi prefill"]
    REV -->|"INTEREST"| D3["Seller notified - confirmation toast with number"]
...
| Success | Dialer/WhatsApp opens (CALL/WHATSAPP) with the number also visible in a sheet; INTEREST shows "विक्रेत्याला कळवले आहे" + the number. |
| Edge | **WhatsApp not installed:** wa.me opens in browser; number remains visible for a manual call. **Listing turns non-APPROVED mid-view:** endpoint 404s; UI swaps state without a crash. **Seller banned seconds earlier:** listings archived by ban → 404. **Two taps racing the 20/day limit:** DB-atomic enforcement; the 21st is rejected. **Desktop without telephony:** `tel:` may fail silently; number visible in the sheet. |
...
8. Repeat taps by the same buyer on the same listing the same day each log a new event (within the 20/day cap); the seller's INTEREST SMS respects the notification caps (no SMS spam).
