...
|---|---|---|---|---|---|
| species | enum | No | `COW\|BUFFALO\|BULL_OX\|GOAT\|SHEEP` (REDA/रेडा retired — not listable). Local copy for the filter form — canonical enum owner is [doc 04](../04-business-rules/README.md) (rule values) + DB `Species` in [doc 07](../07-database/README.md) | Invalid animal type | चुकीचा जनावराचा प्रकार |
...
- [x] Enums match owner docs: species `COW|BUFFALO|BULL_OX|GOAT|SHEEP` (REDA/रेडा retired — not listable), sort `newest|price_asc|price_desc`; server rejects mismatched `breedId`/`species` pairs
