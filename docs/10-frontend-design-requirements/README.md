...
1. PashuSetu (पशुसेतू — "the bridge for livestock") is a **marketplace web app** where farmers in rural Maharashtra sell cows, buffaloes, bulls/oxen, goats and sheep.
...
| HC-07 | **Max 2 levels of navigation depth** | No screen sits more than two levels below a bottom-nav tab root (e.g. Home → Search results → Listing detail). Overlays (sheets, dialogs, photo viewer) do not count as a level. No hamburger menus, no nested settings trees beyond one sub-level (S-15 → S-16 is the deepest settings path). |
...
| HC-14 | **Screens and ids are fixed** | Design exactly the 23 screens + 5 wizard sub-steps of §3 under their `S-xx` ids (Figma pages named "S-07 Listing detail" etc.). You may add **states** of existing screens, never new screens or renumbered ids (doc 06 §8). |
...
- Ids `S-01`…`S-23` (+ `S-10a`…`S-10e`) are the canonical inventory from [../06-user-flows/README.md](../06-user-flows/README.md) §3 — **28 design units** total.
...
2. **Banned account screen** — full-screen block: sober icon, «तुमचे खाते बंद करण्यात आले आहे.» ("Your account has been suspended."), grievance contact **support@pashusetu.in** + helpline number slot, and a button back to public browsing (S-05). No dead end.
...
- **Must contain:** masked target number («+91 98••• ••210») with «नंबर बदला» (change number) link; 6-box OTP input (auto-advance, paste-friendly, WebOTP auto-read as progressive enhancement); 60 s countdown timer; resend control (disabled until timer expiry, then 30 s cooldown between sends); attempt feedback (3 wrong codes invalidate the code); verify button with loading state.
...
- **Must contain:** app bar with logo, language toggle «मराठी | English», notification bell with unread badge (max «9+», logged-in only); search bar; 5 species category chips with icon + Marathi label (गाय · म्हैस · बैल · शेळी · मेंढी); feed of latest APPROVED listing cards (card anatomy in §4, component C-17); bottom nav (HC-08); custom PWA install prompt surface (appears from the 2nd session, dismissible); offline banner slot.
- **Primary action:** tap a species chip or the search bar → S-06. **Secondary:** tap card → S-07; bell → S-14; language toggle; install prompt accept/dismiss.
- **States:** loading (skeleton chips + 4 skeleton cards) · loaded · feed empty (pre-launch rarity: «अजून जाहिराती नाहीत. पहिली जाहिरात तुम्हीच टाका!» / no listings yet — post the first one! + Sell CTA) · feed error (retry, chips still usable) · offline (cached feed + banner) · pull-to-refresh · install prompt visible.
...
- **Must contain:** applied-filter chips row (each removable, with clear-all); result sort control (नवीनतम newest / किंमत कमी→जास्त price low→high / किंमत जास्त→कमी price high→low); listing cards (C-17) in infinite scroll, 20 per page; end-of-results marker; filters bottom sheet containing — species chips, breed picker (dependent on species, resets when species changes, includes «गावठी / स्थानिक» local/crossbred), district picker, min/max price inputs, sort, apply + clear buttons.
- **Primary action:** apply filters / tap a card → S-07. **Secondary:** remove chip; clear all; change sort; share current results URL.
- **States:** initial loading (skeleton cards) · results · paging (skeleton row appended ≥ 300 px before end) · empty (⟦06⟧ «काहीही सापडले नाही. फिल्टर बदलून पुन्हा पहा.» + ⟦06⟧ «फिल्टर काढा» clear CTA + change-filters CTA) · filter validation error (min > max: inline under price fields, apply blocked) · network error (inline retry row appended; loaded cards kept) · offline (cached results + banner) · end of results (⟦06⟧ «सर्व जाहिराती पाहिल्या» — a visible stop, not a spinner) · deep-link hydration (chips prefilled from URL before first paint) · back-restore (scroll + filters restored after returning from S-07).
- **Navigation:** In — S-05 chips/search, deep link `/listings?…`, S-09 "other listings". Out — S-07; back → S-05.
- **Marathi copy:** as above; «फिल्टर» (filters) · «लावा» (apply) · «सर्व काढा» (clear all).
...
- **Must contain:** 5-slot photo grid; add-photo tile offering camera and gallery («कॅमेरा» / «गॅलरी»); per-photo tile with upload progress, retry on failure, delete, drag-to-reorder **plus non-drag reorder alternative** (§6), cover marker on position 1 («मुख्य फोटो» main photo); the 3-photo nudge; link to a photo-tips sheet.
- **Primary action:** «पुढे» (enabled at ≥ 1 uploaded photo) → S-10d.
- **States:** empty (0 photos, next disabled) · uploading (per-tile progress) · upload failed (per-tile retry; others unaffected) · file rejected client-side («फोटो खूप मोठा आहे. दुसरा फोटो निवडा.» / photo too big, choose another — also for wrong format) · full (5/5 — add tile hidden) · reordering · tips sheet open («उन्हात, पूर्ण जनावर दिसेल असा फोटो काढा» / shoot in daylight with the full animal visible) · offline (adding disabled with message; existing photos kept).
...
- **States:** loading skeleton · loaded · empty («अजून एकही जाहिरात जतन केलेली नाही. आवडलेल्या जाहिरातीवर जतन दाबा.» / nothing saved yet — tap Save on a listing you like + browse CTA → S-05) · unavailable-item variant · paging · error retry · offline (cached list; toggling disabled).
- **Navigation:** In — Favorites tab (login-walled). Out — S-07.
...
- **Must contain:** editable name, district, taluka, village, role flags (same components as S-04); phone displayed read-only with «फोन नंबर बदलता येत नाही» (phone number cannot be changed); rows — भाषा (language → S-16), सूचना (notifications → S-14), मदत व तक्रार (help & grievance: helpline number + support@pashusetu.in), नियम व अटी (T&C), गोपनीयता (privacy); logout «बाहेर पडा» with confirmation; app version line.
- **Primary action:** save edits. **Secondary:** navigation rows; logout.
- **States:** loading · default · editing/saving per field group · save error · logout confirmation («बाहेर पडायचे का?» / log out?) · offline (read-only) · anonymous variant (tab tapped while logged out → login wall).
...
- **Must contain:** centered auth card reusing C-03/C-04 (phone + OTP); verifying state; access-denied page (no admin data visible, link back to the public app); admin shell after success (sidebar: Pending · Reports · Users · Stats + logout).
...
- **Must contain:** metric tiles — pending count + oldest-pending age, approvals/rejections (7 d / 30 d), new users & listings, listings by status, interest events, inquiry rate, open reports, moderation turnaround median + p95; breakdown tables/simple charts by district and species; audit-log table (admin, action, target, reason, timestamp) with filters (admin, action, date range) and cursor paging; drill-through links to S-19/S-21.
...
| C-10 | Species picker | 5-item icon grid (large), chip row (compact, for filters) | default · selected · disabled |
...
| C-13 | Bottom sheet | filters, seller snippet, login wall, photo tips, contact confirmation | opening · open · scrollable-content · closing (scrim tap, close button, back) |
...
| C-19 | Photo uploader tile | add tile (camera/gallery), photo tile | empty · picking · **uploading with progress %** · uploaded · **failed with retry** · delete confirm · cover-marked · reorder (drag + button alternative) |
...
| C-23 | App bar | home (logo + language + bell), sub-screen (back + title), contextual (share on S-07) | default · with unread bell badge («9+» cap) · offline banner attached |
...
| C-26 | Empty-state block | **illustration slot** + one sentence + CTA | per-screen content variants (S-05/06/11/13/14/19/21/22) |
...
| All business rules and limits (photos 1–5, 10 active listings, 30-day expiry, 24 h SLA, rate limits, report reasons, status set and transitions) | [../04-business-rules/README.md](../04-business-rules/README.md) |
